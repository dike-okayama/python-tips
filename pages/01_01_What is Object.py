...
st.info('''
リテラルとは，組み込み型のオブジェクトを生成するための記法です．例えば文字列型であれば，クォーテーション`''`や
ダブルクォーテーション`""`で囲み，整数型であれば0以外から始まる数字を並べると，オブジェクトが生成できます．
''')
st.markdown('---')
...
st.info('''
実際のところ，Pythonでは直接的なメモリアクセスは推奨されません．
''')
...
st.write('''\
変数に対して「値を格納する箱」のようなイメージを持っていましたが，Pythonの文脈においてこのイメージは不正確です．
''')
...
st.write('''
加えて前述のとおり，オブジェクトが型と値をもつ透明な箱であることを踏まえると，実際には**変数は値を格納する箱ではなく，オブジェクトに紐づけられたタグ**であるといえます．

以上より，変数とオブジェクトを次のように捉えると一貫性を保てます．
''')
...
st.write('''
簡潔に表現すると，変数は**オブジェクトに対する参照**です．作ったオブジェクトがメモリの波に飲み込まれれる前に，変数を通してオブジェクトへの参照を保持することで，オブジェクトへアクセスすることができます．
''')
...
st.write('''\
上記の例ではobjという変数を利用してオブジェクトへの参照を保持していますが，以下のようにオブジェクトへの参照が保持されず，オブジェクトへ到達不可能になると，オブジェクトはガベージコレクトされます．
...
st.caption('''
2行目でリテラルを利用して'Hello World'という値をもつ文字列オブジェクトが生成されましたが，それ以降オブジェクトに対して操作することは不可能です．
''')
st.caption("""
