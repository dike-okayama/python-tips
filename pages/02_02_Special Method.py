...
IDEでもよくサジェストされますが，`__xxx__`のようなメソッドは**特殊メソッド**(special method)と呼ばれ，組み込み関数の`len`や`abs`，または演算子`+`や`-`，`[]`等の振る舞いを定義します．
''')
...
st.info('''
前後の2つのアンダーバーは，**ダンダー**(**d**ouble **under** bar)と呼ばれます．
''')

st.header('2. 特殊メソッドの使い方')

st.header('3. 特殊メソッド一覧')
